...
function AgentCard({ agent }) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isIdle ? '#F9FAFB' : '#FFFFFF', // Light grey for standby
        borderColor: isRunning ? '#F59E0B' : (isDone ? '#E5E7EB' : (isFailed ? '#FECACA' : '#E5E7EB')),
        boxShadow: isRunning ? `0 0 0 2px rgba(245,158,11,0.4)` : '0 1px 2px rgba(0,0,0,0.05)',
        borderLeftWidth: '5px',
        borderLeftColor: isRunning ? '#F59E0B' : (isDone ? '#16A34A' : (isFailed ? '#DC2626' : '#D1D5DB')),
      }}
    >
      {isRunning && (
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center h-full">
        ...
        <div className="mt-1 flex items-center">
          ...
          >
            {isRunning ? 'Analyzing' : isDone ? 'Complete' : isFailed ? 'Failed' : 'WAITING'}
          </span>
        </div>
...
export default function AgentActivityFeed({ agents }) {
