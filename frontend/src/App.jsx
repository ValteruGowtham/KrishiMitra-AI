...
function Header({ onOpenAudit }) {
  return (
    <header className="bg-primary bg-grain text-white px-6 py-4 flex items-center justify-between shadow-lg z-10 relative shrink-0">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-accent rounded-xl flex items-center justify-center shadow-md">
          <Leaf className="w-6 h-6 text-primary-dark" />
        </div>
        <div>
          <h1 className="text-xl font-bold tracking-tight">KrishiMitra AI</h1>
          <p className="text-xs text-white/80 font-medium tracking-wide">
            कृषि मित्र — Your Trusted Farming Advisor
          </p>
        </div>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <StatusBadge label="8 AI Agents" icon={<Activity className="w-3.5 h-3.5" />} />
        <StatusBadge label="Compliance" icon={<Shield className="w-3.5 h-3.5" />} />
        <button 
          onClick={onOpenAudit}
          className="flex items-center gap-1.5 bg-black/20 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide border border-white/10 shadow-sm hover:bg-black/30 transition-colors cursor-pointer"
        >
          <span className="text-accent-light"><FileText className="w-3.5 h-3.5" /></span>
          Audit Trail
        </button>
      </div>
    </header>
...
function StatusBadge({ label, icon }) {
  return (
    <span className="flex items-center gap-1.5 bg-black/20 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide border border-white/10 shadow-sm">
...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col bg-surface font-sans antialiased text-dark overflow-hidden">
      <Header onOpenAudit={() => setIsAuditOpen(true)} />
